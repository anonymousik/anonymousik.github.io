#!/usr/bin/env bash
# ╔═══════════════════════════════════════════════════════════════════════════════╗
# ║  GW4 Pro Optimizer Suite  v6.0.0                                            ║
...
# ║    gw4_optimizer_v6.sh)                                                    ║
# ║                                                                              ║
# ║  Changelog: v6.0 → Shizuku/rish support, I/O scheduler, sched_boost fix,   ║
# ║             batterystats, JIT disable_zygote, dirty_regions,                ║
# ║             WAKE_LOCK shealth, kompilacja cold-start benchmark             ║
...
readonly DAEMON_INTERVAL=300      # sekund między cyklami monitoringu
...
# I/O scheduler — v6.0
readonly IO_SCHED_OPT="deadline"   # lepszy niż cfq dla zRAM i NAND na WearOS
readonly IO_SCHED_DEFAULT="cfq"

# sched_boost — v6.0 (raport str.1: faworyzuje procesy tła kosztem UI thread)
readonly SCHED_BOOST_OPT=1
readonly SCHED_BOOST_PATH="/proc/sys/kernel/sched_boost"

# Thresholds diagnostyki v6.0 (z raportu styczeń 2026)
readonly DIAG_JANKY_THRESHOLD=5     # > 5% janky → alarm
readonly DIAG_BATTERY_IDLE_THRESHOLD=2  # > 2% / h idle → alarm
readonly DIAG_APP_LAUNCH_THRESHOLD=800  # > 800ms am start -W → alarm

# ─── Target profiles (v5.2) ──────────────────────────────────────────────────
# TARGET="watch"  → Exynos W920, 2-core A55, 1.5GB (default)
# TARGET="phone"  → Exynos 2100/2200, big.LITTLE 8-core, 8-12GB
TARGET="watch"

# Exynos 2100/2200 (Galaxy S21/S22) — parametry dla 8-core big.LITTLE
readonly PHONE_SCHED_UP_OPT=400        # mniej agresywne (więcej rdzeni absorbuje)
readonly PHONE_SCHED_DOWN_OPT=8000
readonly PHONE_SCHED_LAT_OPT=3500000
readonly PHONE_VM_SWAP_OPT=80          # więcej RAM → mniej agresywna redukcja
...
# ART — v5.1 extended (analiza regresji Android 16)
readonly ART_HEAPTARGET=0.75
readonly ART_HEAPMAXFREE="8m"
ART_HEAPSIZE="128m"
ART_HEAPGROWTHLIMIT="80m"
readonly ART_DEX2OAT_THREADS="1"     # nie rywalizuje z UI na 2-core A55

# MGLRU — v5.1 (Multi-Generational LRU, Android 16)
readonly MGLRU_MIN_TTL_MS=1000       # strony zyja min. 1s zanim uznane za zimne

# device_config namespaces (Android 16 API) — v5.1
...
# Parsowanie argumentów CLI (v5.2)
# Użycie: bash gw4_optimizer_v5_2.sh [--target watch|phone] [--daemon] [--ip X.X.X.X]
_parse_args() {
    local ip_override=""
    while [[ $# -gt 0 ]]; do
        case "$1" in
...
_detect_device() {
    ...
    # Weryfikacja modelu + auto-detect target profile
...
# Ustaw zmienne runtime zgodnie z wykrytym profilem
_load_device_profile() {
...
# Zapisz profil bieżącej sesji (co zostało zastosowane)
...
# Wczytaj i zaproponuj profil z poprzedniej sesji
...
# Ciche wersje fixów (bez interakcji) dla profile replay
    ...
    case "${_c}" in
        1)
            ...
            _apply "shealth autodetect off" \
                ...
                2>/dev/null || true
            # v6.0: shealth WAKE_LOCK (raport styczeń 2026)
...
# ───────────────────────────────────────────────────────────────────────────────
# §19b  DIAGNOSTYKA RÓŻNICOWA  (v5.2 preview)
#        Zbiera snapshot PRZED i PO fixach, generuje DIFF_REPORT.txt
# ───────────────────────────────────────────────────────────────────────────────
...
_snapshot_collect() {
    ...
    local snap="${LOG_DIR}/snap_${label}_$(date +%H%M%S).txt"
    {
        ...
        # gfxinfo janky (systemui) — do diff
        local janky
...
# ───────────────────────────────────────────────────────────────────────────────
# §15a  FIX H — device_config ART / RUNTIME FLAGS  (NOWE v5.1)
#        Źródło: Analiza regresji Android 16 — sekcja A (JIT Thrashing)
#
#  PROBLEM: OTA One UI 8.0 nie wywołuje dex2oat z flagami runtime_native_boot.
#  Urządzenie działa w trybie JIT → CPU spikes przy każdym otwarciu aplikacji.
#  ROZWIĄZANIE: device_config (Android 16 API, bez roota) + dex2oat priority.
# ───────────────────────────────────────────────────────────────────────────────

_fix_device_config_art() {
    hdr "Fix H: ART device_config + runtime flags (v5.1)"
    _backup_settings

    # Odczyt bieżących flag
    local cur_uffd cur_gcc
    cur_uffd="$(_sh_retry "device_config get ${DC_RUNTIME_NS} enable_uffd_gc" 2>/dev/null || echo 'N/A')"
    cur_gcc="$(  _sh_retry "device_config get ${DC_RUNTIME_NS} enable_generational_cc" 2>/dev/null || echo 'N/A')"

    printf "\n"
    printf "  ${GRAY}┌──────────────────────────────────────────────────────────┐${C0}\n"
    printf "  ${GRAY}│${C0}  ${WHITE}%-38s${C0}  %-16s  ${GRAY}│${C0}\n" \
           "enable_uffd_gc:" "${cur_uffd}"
    printf "  ${GRAY}│${C0}  ${WHITE}%-38s${C0}  %-16s  ${GRAY}│${C0}\n" \
           "enable_generational_cc:" "${cur_gcc}"
    printf "  ${GRAY}│${C0}                                                            ${GRAY}│${C0}\n"
    printf "  ${GRAY}│${C0}  ${YELLOW}OTA One UI 8.0 nie uruchamia dex2oat z flags AOT${C0}       ${GRAY}│${C0}\n"
    printf "  ${GRAY}│${C0}  ${YELLOW}Efekt: JIT thrashing → CPU 100%% przy otwarciu aplikacji${C0}  ${GRAY}│${C0}\n"
    printf "  ${GRAY}└──────────────────────────────────────────────────────────┘${C0}\n\n"

    printf "  ${CYAN}1${C0}) ${GREEN}Pełna korekcja ART flags${C0} ${GRAY}— device_config + dex2oat priority${C0}\n"
    printf "     ${GRAY}├─ enable_uffd_gc=true    (efektywniejsze GC na 2-core)${C0}\n"
    printf "     ${GRAY}├─ enable_generational_cc (redukuje GC pauses)${C0}\n"
    printf "     ${GRAY}├─ dex2oat-threads=1      (nie rywalizuje z UI)${C0}\n"
    printf "     ${GRAY}└─ compile-layouts         (layout inflation cache)${C0}\n"
    printf "  ${CYAN}2${C0}) Tylko dex2oat priority ${GRAY}— bezpieczne, szybkie${C0}\n"
    printf "  ${CYAN}3${C0}) device_config dump      ${GRAY}— podgląd aktualnych flag${C0}\n"
    printf "  ${CYAN}0${C0}) Anuluj\n\n"
    read -r -p "  > " _c

    case "${_c}" in
        1)
            _bar "UFFD GC flag" 15
            _apply "enable_uffd_gc=true" \
                "device_config put ${DC_RUNTIME_NS} enable_uffd_gc true"
            printf "\n"

            _bar "Generational CC flag" 30
            _apply "enable_generational_cc=true" \
                "device_config put ${DC_RUNTIME_NS} enable_generational_cc true"
            # v6.0: disable_jit_zygote (analiza marzec 2026)
            _apply "disable_jit_zygote=false" \
                "device_config put ${DC_RUNTIME_NS} disable_jit_zygote false" \
                2>/dev/null || true
            printf "\n"

            _bar "dex2oat priority" 50
            _apply "dex2oat-cpu-set=0" "setprop dalvik.vm.dex2oat-cpu-set 0"
            _apply "dex2oat-threads=${ART_DEX2OAT_THREADS}" \
                "setprop dalvik.vm.dex2oat-threads ${ART_DEX2OAT_THREADS}"
            _apply "image-dex2oat-threads=${ART_DEX2OAT_THREADS}" \
                "setprop dalvik.vm.image-dex2oat-threads ${ART_DEX2OAT_THREADS}"
            printf "\n"

            _bar "Heap tuning (OOM guard)" 68
            _apply "heaptargetutilization" \
                "setprop dalvik.vm.heaptargetutilization ${ART_HEAPTARGET}"
            _apply "heapmaxfree"       "setprop dalvik.vm.heapmaxfree ${ART_HEAPMAXFREE}"
            _apply "heapsize"          "setprop dalvik.vm.heapsize ${ART_HEAPSIZE}"
            _apply "heapgrowthlimit"   "setprop dalvik.vm.heapgrowthlimit ${ART_HEAPGROWTHLIMIT}"
            printf "\n"

            _bar "compile-layouts" 85
            _sh_retry "cmd package compile-layouts -a" 2>/dev/null || \
                skip "compile-layouts (nieobsługiwane na tym SDK)"
            printf "\n"

            _bar "Gotowe" 100; printf "\n"
            ok "ART device_config flags zastosowane"
            sub "Zalecane: uruchom Fix G (pm compile speed-profile) po tym kroku"
            sub "Weryfikacja: adb shell dumpsys package APP | grep compilerFilter"
            ;;
        2)
            _apply "dex2oat-cpu-set=0"  "setprop dalvik.vm.dex2oat-cpu-set 0"
            _apply "dex2oat-threads=${ART_DEX2OAT_THREADS}" \
                "setprop dalvik.vm.dex2oat-threads ${ART_DEX2OAT_THREADS}"
            ok "dex2oat priority ustawiony — niski, nie blokuje UI"
            ;;
        3)
            info "device_config dump: ${DC_RUNTIME_NS}"
            _sh_retry "device_config list ${DC_RUNTIME_NS}" 2>/dev/null | while IFS= read -r line; do
                printf "  ${GRAY}%s${C0}\n" "${line}"
            done
            printf "\n"
            info "device_config dump: ${DC_SF_NS}"
            _sh_retry "device_config list ${DC_SF_NS}" 2>/dev/null | while IFS= read -r line; do
                printf "  ${GRAY}%s${C0}\n" "${line}"
            done
            ;;
        0) return ;;
...
# ───────────────────────────────────────────────────────────────────────────────
# §15b  FIX P — TRWAŁOŚĆ PARAMETRÓW JĄDRA  (NOWE v5.2)
#        Źródło: Plan aktualizacji v5.2 — persystencja schedutil/swappiness
#
#  PROBLEM: Parametry /proc/sys/ i /sys/devices/ resetują się przy każdym
#           restarcie urządzenia. Fix D (schedutil) i Fix E (swappiness) muszą
#           być ponownie stosowane po każdym wybudzeniu.
#  ROZWIĄZANIE A: Generator profilu Tasker (XML) — importuj raz, działa zawsze.
#  ROZWIĄZANIE B: Boot hook root (/data/adb/service.d/) — wymaga Magisk/KernelSU.
#  ROZWIĄZANIE C: Daemon na hoście — nasłuchuje ADB, stosuje fixy po restarcie.
# ───────────────────────────────────────────────────────────────────────────────
...
_fix_persistence() {
    ...
    printf "  ${GRAY}Dotyczy: Fix D (PELT), Fix E (swappiness), Fix H (MGLRU)${C0}\n\n"
...
<!-- GW4 Optimizer — Tasker Boot Profile v${VERSION} -->
<!-- Import: Tasker → Profile → Import XML -->
<!-- Wygenerowano: $(date) | Urządzenie: ${DEVICE_MODEL:-unknown} | IP: ${DEVICE} -->
<TaskerData sr="" dvi="1" tv="6.3.13">
  ...
  <Profile sr="prof0" ve="2">
    ...
    <Task sr="task0">
      ...
      <!-- Poczekaj 15s na stabilizację ADB po boot -->
      <Action sr="act0" ve="7">
        <code>30</code>
        <Int sr="arg0" val="15"/>
      </Action>
      ...
      <!-- Fix D: schedutil up_rate_limit_us = ${SCHED_UP_OPT} -->
      <Action sr="act2" ve="7">
        <code>123</code>
      ...
      <!-- Fix D: schedutil down_rate_limit_us = ${SCHED_DOWN_OPT} -->
      <Action sr="act3" ve="7">
        <code>123</code>
      ...
      <!-- Fix D: sched_latency_ns = ${SCHED_LAT_OPT} -->
      <Action sr="act4" ve="7">
        <code>123</code>
      ...
      <!-- Fix E: extra_free_kbytes = ${VM_EXTRA_FREE_KB} -->
      <Action sr="act6" ve="7">
        <code>123</code>
      ...
      <!-- Fix E: MGLRU min_ttl_ms = ${MGLRU_MIN_TTL_MS} -->
      <Action sr="act7" ve="7">
        <code>123</code>
...
_install_boot_hook_root() {
    ...
    if [[ "${uid}" != "0" ]]; then
        ...
        sub "Alternatywa bez roota: użyj opcji 1 (Tasker) lub 3 (daemon)"
        return 1
    fi
...
# Wygenerowano: $(date) | Model: ${DEVICE_MODEL}
#
# Uruchamiany automatycznie przez Magisk/KernelSU przy każdym boot.

# Poczekaj na stabilizację systemu (SurfaceFlinger, schedulery)
sleep 20
...
HOOKEOF
    ...
    printf "%s" "${hook_content}" | _sh "cat > ${BOOT_HOOK_PATH}" 2>/dev/null && {
        ...
        sub "Parametry będą automatycznie przywracane po każdym restarcie"
        sub "Log: /data/local/tmp/gw4_boot.log"
        warn "Wymaga Magisk/KernelSU — bez niego skrypt nie zostanie uruchomiony"
    } || {
        err "Nie można zapisać do ${BOOT_HOOK_PATH}"
        sub "Sprawdź czy Magisk/KernelSU jest zainstalowany i katalog /data/adb/service.d/ istnieje"
    }
}
...
_watch_daemon_mode() {
    ...
    printf "  ${GRAY}• Po każdym wykryciu zegarku stosować Fix D+E+H (schedutil+swap+MGLRU)${C0}\n"
...
# Pętla daemona (uruchamiana w tle)
_daemon_worker() {
    ...
    while true; do
        ...
        # Sprawdź czy zegarek nadal podłączony
        local state
        state="$(adb -s "${DEVICE}" get-state 2>/dev/null || echo 'offline')"

        if [[ "${state}" == "device" ]]; then
            # Sprawdź czy parametry wymagają korekty
            local cur_swap
            ...
            if [[ "${cur_swap}" != "${VM_SWAP_OPT}" ]]; then
                ...
                # Auto-fix: tylko D + E (bezpieczne, bez interakcji)
                ...
                echo "[$(date)] Cykl ${cycle}: fixy D+E zastosowane" >> "${DAEMON_LOG}"
            else
...
_daemon_status() {
    if [[ -f "${DAEMON_PID_FILE}" ]]; then
        ...
        if kill -0 "${pid}" 2>/dev/null; then
            ...
            sub "Uruchom ponownie: opcja P → 3"
        fi
    else
        info "Daemon nie jest uruchomiony"
        sub "Uruchom: opcja P → 3"
    fi
    printf "\n"
}
...
# ───────────────────────────────────────────────────────────────────────────────
# §15c  FIX I — I/O SCHEDULER + SCHED_BOOST  (NOWE v6.0)
#        Źródło: Raport regresji styczeń 2026 str.1 (sched_boost + cpufreq)
#                + Analiza regresji marzec 2026 (I/O deadline)
#
#  PROBLEM sched_boost: One UI 8.0 faworyzuje procesy tła (Google Play Services)
#  kosztem wątku UI (SurfaceFlinger). Na 2-core A55 = każdy cykl jest krytyczny.
#  PROBLEM I/O: Domyślny scheduler CFQ (Completely Fair Queuing) na WearOS 6.0
#  nie jest optymalny dla NAND flash zegarka — scheduler "deadline" zapewnia
#  deterministyczne latency dla operacji I/O podczas zRAM swap.
# ───────────────────────────────────────────────────────────────────────────────
    ...
    case "${_c}" in
        1|2)
            ...
            if _sh "echo ${SCHED_BOOST_OPT} > ${SCHED_BOOST_PATH}" 2>/dev/null; then
                ...
                sub "SurfaceFlinger i wątki renderowania dostaną CPU przed procesami tła"
            else
                skip "sched_boost (niedostępne bez roota)"
                # Fallback: podniesie priorytety przez settings
                _apply "high_priority_render_thread=1" \
                    "settings put global high_priority_render_thread 1"
            fi
            [[ "${_c}" == "2" ]] && return ;;& # fallthrough do IO tylko dla opcji 1
        1|3)
            ...
            if [[ ${applied} -gt 0 ]]; then
                ...
                sub "Deterministyczne I/O latency — redukcja freeze przy zRAM swap"
            else
...
# ───────────────────────────────────────────────────────────────────────────────
# §15d  FIX J — SHIZUKU / RISH  (NOWE v6.0)
#        Źródło: Shizuku_Deployer_secferro_sh.txt (SecFerro Division v2.1.1-FIX)
#
#  CO TO JEST SHIZUKU:
#  Shizuku umożliwia aplikacjom korzystanie z uprawnień systemowych (ADB-level)
#  BEZ roota, przez mechanizm "rish" (Remote Interactive Shell).
#  Na WearOS 6.0 pozwala na trwałe zastosowanie fixów przez aplikacje zainstalowane
#  na zegarku — eliminuje potrzebę kabla ADB po pierwszej konfiguracji.
#
#  RISH: skrypt proxy który uruchamia shell z uprawnieniami Shizuku
#  Ścieżka: /data/local/tmp/rish
# ───────────────────────────────────────────────────────────────────────────────
...
_fix_shizuku() {
    ...
    # Sprawdź czy Shizuku jest zainstalowane
...
_shizuku_deploy_rish() {
    ...
    # Bazuje na: Shizuku_Deployer_secferro_sh.txt (SecFerro Division v2.1.1-FIX)

    # Krok 1: Znajdź APK Shizuku na urządzeniu
    local pkg_path
...
# ───────────────────────────────────────────────────────────────────────────────
# §15e  FIX K — DIAGNOSTYKA ROZSZERZONA  (NOWE v6.0)
#        Źródło: Raport regresji styczeń 2026 (progi alarmowe)
#                + ANALIZA_REGRESJI_Android16.md (Krok diagnostyki)
#
#  Nowe wskaźniki: compilationReason, am start -W timing, batterystats idle drain,
#  LowMemoryKiller events, dirty_regions, disable_jit_zygote verification
# ───────────────────────────────────────────────────────────────────────────────
...
_diag_extended() {
    ...
    # ── 2. App launch timing (am start -W) ────────────────────────────────
...
main() {
    ...
    # Zaproponuj profil z poprzedniej sesji
    _offer_session_profile
