...
# ║    gw4_optimizer_v5.sh)                                                      ║
# ║                                                                              ║
# ║  Changelog: v5.1 → Fix H (ART device_config, MGLRU, SF device_config),     ║
# ║             compile-layouts, dex2oat priority, ART verification             ║
...
_menu() {
    ...
    printf "\n"
    printf "  ${LCYAN}H${C0}) ART device_config    ${GRAY}~3s  │ ${LGREEN}NOWE v5.1${C0}${GRAY} JIT thrashing fix${C0}\n"
    printf "\n"
    printf "  ${WHITE}${BOLD}─── NARZĘDZIA ──────────────────────────────────────────────────────${C0}\n"
    printf "  ${LCYAN}8${C0}) Diagnostyka systemu   ${GRAY}zbierz raporty (Toybox-safe)${C0}\n"
    printf "  ${LCYAN}0${C0}) Diagnostyka różnicowa ${GRAY}${LGREEN}NOWE v5.2${C0}${GRAY} snapshot PRZED/PO${C0}\n"
