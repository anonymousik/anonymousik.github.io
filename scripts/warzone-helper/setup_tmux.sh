...
# --- TERMUX ADAPTIVE RECTIFY & RUNNER ---
# Automatyczne czyszczenie, aktualizacja i uruchamianie projektu pod Termuxem

set -e

echo "=== [TERMUX RECTIFY PROCESS] ==="

# 1. Sprawdzenie katalogu roboczego
TARGET_DIR="/data/data/com.termux/files/home/warzone-helper/warzone-helper/warzone-helper"

if [ -d "$TARGET_DIR" ]; then
    echo "✔ Odnaleziono katalog projektu: $TARGET_DIR"
    cd "$TARGET_DIR"
else
    echo "⚠ Katalog predefiniowany nie istnieje. Szukam bieżącego..."
    # Sprawdzenie czy jesteśmy już wewnątrz folderu z package.json
    if [ -f "package.json" ]; then
        echo "✔ Znaleziono package.json w bieżącym katalogu."
    else
        echo "❌ Błąd: Nie odnaleziono poprawnego katalogu projektu React."
        exit 1
    fi
fi

# 2. Usuwanie błędnego pliku i upewnienie się o poprawnej strukturze React
echo "-> Nadpisywanie src/App.jsx czystym kodem komponentu..."
mkdir -p src

# Kopiowanie kodu React bezpośrednio do pliku App.jsx
cat << 'EOF' > src/App.jsx
  ...
  return (
    <div className="w-full max-w-xs mx-auto p-4 bg-zinc-950/90 rounded-2xl border border-zinc-800 flex flex-col items-center">
      ...
      <svg viewBox="0 0 400 280" className="w-full h-auto max-h-[160px] text-zinc-700 fill-zinc-900 stroke-zinc-700 stroke-[4]">
        ...
        <text x="250" y="215" className="text-[10px] font-bold text-center fill-amber-500 font-sans" textAnchor="middle">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
          ...
          <section className="lg:col-span-4 p-6 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl flex flex-col justify-between space-y-4">
            <div>
              ...
              <p className="text-[11px] text-zinc-500 leading-normal font-medium text-justify font-sans">Wygeneruj unikalny link profilu bezpośrednio skorelowany z platformą <span className="text-amber-500 font-bold">Anonymousik.is-a.dev/scripts/warz</span>.</p>
            </div>
...
# 3. Instalacja brakujących zależności oraz czyszczenie cache
echo "-> Instalacja i weryfikacja pakietów NPM..."
npm install --no-audit --no-fund

# 4. Nadanie prawidłowej konfiguracji HMR dla lokalnego serwera w Termux
echo "-> Optymalizacja vite.config.js pod środowisko mobilne i Termux..."
...
export default defineConfig({
  plugins: [react()],
  server: {
    host: '0.0.0.0', # Zezwala na połączenia z innych urządzeń w sieci lokalnej (np. telefon/PC)
    port: 5173,
    hmr: {
      overlay: false # Wyłącza nakładkę błędów na ekranie przy drobnych ostrzeżeniach
    }
  }
})
EOF

# 5. Odpalenie serwera deweloperskiego
echo "=========================================================="
echo " ✔ PROJEKT ZOSTAŁ NAPRAWIONY I JEST GOTOWY DO STARTU!     "
echo "=========================================================="
echo "Serwer deweloperski zostanie teraz uruchomiony."
echo "Możesz otworzyć aplikację w przeglądarce pod adresem:"
echo "👉 http://localhost:5173"
echo "lub używając adresu IP swojego telefonu w sieci Wi-Fi."
echo "=========================================================="
