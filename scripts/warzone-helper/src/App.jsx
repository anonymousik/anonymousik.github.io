...
import { 
  ...
  Sliders, ShieldAlert, Search, Download, HelpCircle, Eye, EyeOff,
  SlidersHorizontal as TuningIcon, CheckCircle2, AlertCircle, ExternalLink
} from 'lucide-react';
...
// --- CUSTOM INTERACTIVE CONTROLLER COMPONENT ---
...
// --- VISUAL INTERACTIVE FOV SIMULATOR ---
const FovSimulator = ({ fovValue, adsFov }) => {
  return (
    <div className="w-full bg-zinc-950 rounded-2xl border border-zinc-800 p-4 overflow-hidden relative">
      ...
      {/* Screen area representing the camera */}
      <div className="relative aspect-video w-full bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden flex items-center justify-center">
        {/* Grid lines to represent environment reference */}
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#1f2937_1px,transparent_1px),linear-gradient(to_bottom,#1f2937_1px,transparent_1px)] bg-[size:4rem_4rem] opacity-20" />
        
        {/* Simulated Walls/Pillars on the side that disappear at low FOV */}
        <div 
        ...
        <div 
          ...
          style={{ 
            // Scale enemy based on FOV. Higher FOV = Enemy is smaller and further (Affected)
        ...
        {/* Visual indicators of Peripheral Vision */}
      ...
      <p className="text-[10px] text-zinc-500 mt-2 leading-relaxed">
        Suwak wpływa na odczuwanie prędkości. Im wyższe pole widzenia (FOV), tym mniejsza wydaje się sylwetka wroga, ale ruchy postaci sprawiają wrażenie znacznie szybszych i dynamiczniejszych.
      </p>
    </div>
  );
};
...
const App = () => {
  ...
  // Sharing & Toast Notification States
  ...
  // ZSSP Share Config: Encode to Base64 and write to clipboard (using the script domain path)
  const generateShareLink = () => {
    try {
      const configStr = btoa(JSON.stringify(settings));
      // Base link pointed to your dedicated domain script path
  ...
  const handleImportConfig = () => {
    try {
      ...
      // Handle paste of whole URL containing config query
  ...
  // Export full settings configuration as markdown txt cheat sheet
  ...
  // Filtered settings list for tactical global search
  ...
  return (
    <div className="min-h-screen bg-[#07080c] text-zinc-300 font-sans selection:bg-amber-500/30 pb-20 relative overflow-hidden">
      ...
      {/* Background Military Sci-Fi Grid Elements */}
      ...
      {/* TOP BRAND NAVIGATION BAR (Anonymousik Portal) */}
      <nav className="w-full bg-black/90 border-b border-zinc-900 px-4 py-2.5 flex flex-col sm:flex-row justify-between items-center gap-3 relative z-50 text-[11px] font-mono tracking-wider">
        <div className="flex items-center gap-2">
          ...
          <a 
            href="https://anonymousik.is-a.dev" 
            target="_blank" 
            rel="noopener noreferrer" 
            className="text-white hover:text-amber-500 transition-colors font-extrabold flex items-center gap-1"
          >
        ...
        <div className="flex items-center gap-4 text-zinc-400">
          ...
          <a 
            href="https://github.com/anonymousik" 
            target="_blank" 
            rel="noopener noreferrer"
      ...
      {toast.show && (
        <div className={`fixed top-6 right-6 z-50 p-4 rounded-xl shadow-2xl flex items-center gap-3 border animate-fadeIn transition-all max-w-sm ${
          toast.type === 'success' 
            ? 'bg-zinc-900/95 border-emerald-500 text-emerald-300' 
            : 'bg-zinc-900/95 border-red-500 text-red-300'
        }`}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl md:text-2xl font-black text-white tracking-widest uppercase">
                  WARZONE <span className="text-amber-500">HQ</span>
                </h1>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button 
              ...
              className="px-4 py-2 bg-zinc-950/80 border border-zinc-800 hover:border-amber-500 text-xs font-bold rounded-lg flex items-center gap-2 transition-all text-zinc-400 hover:text-white"
            >
            ...
            <div className="flex items-center bg-zinc-950/80 p-1 rounded-xl border border-zinc-800">
              <button 
                ...
                className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 ${!isAdvanced ? 'bg-zinc-800 text-white shadow-md' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                Początkujący
              </button>
              <button 
                onClick={() => setIsAdvanced(true)}
                className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 flex items-center gap-1 ${isAdvanced ? 'bg-amber-500/10 text-amber-500 border border-amber-500/30' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
      ...
      {showSurvey ? (
        <section className="max-w-4xl mx-auto px-4 py-12 animate-fadeIn">
          <div className="bg-zinc-900/90 border border-zinc-800/80 rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden backdrop-blur-lg">
            ...
            <h2 className="text-2xl md:text-3xl font-black text-white mb-8 uppercase tracking-tight">
                ...
                return (
                  <button
                    ...
                    className={`p-5 rounded-2xl text-left border transition-all duration-200 flex flex-col justify-between ${
                      isSelected 
                        ? 'bg-amber-500/10 border-amber-500 text-white shadow-[0_0_25px_rgba(245,158,11,0.15)]' 
                        : 'bg-zinc-950/40 border-zinc-800 hover:bg-zinc-900/30 hover:border-zinc-700 text-zinc-400'
                    }`}
                  >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-zinc-800/80 pt-6">
              <button 
                ...
                className="text-xs font-bold text-zinc-500 hover:text-zinc-300 transition-colors uppercase tracking-wider font-mono"
              >
                Pomiń kreator (Zostaw Domyślne)
              </button>
              
              <button
                onClick={nextSurveyStep}
                className="w-full sm:w-auto px-8 py-3.5 bg-amber-500 hover:bg-amber-600 text-zinc-950 font-black text-xs uppercase tracking-widest rounded-xl flex items-center justify-center gap-2 transition-all shadow-[0_4px_20px_rgba(245,158,11,0.3)]"
              >
      ...
      <main className={`max-w-7xl mx-auto px-4 py-8 transition-opacity duration-300 ${showSurvey ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
        ...
        {/* TACTICAL SEARCH & GLOBAL TELEMETRY AREA */}
        <div className="mb-8 p-4 bg-zinc-900/50 border border-zinc-800/80 rounded-2xl flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="relative w-full lg:max-w-md">
            ...
            <input 
              type="text"
              placeholder="Wyszukaj parametr (np. deadzone, czułość, fov)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 bg-zinc-950 border border-zinc-800 focus:border-amber-500/80 rounded-xl text-sm placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-amber-500/30 text-white"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2 items-center w-full lg:w-auto justify-end">
            <button 
              ...
              className="px-4 py-2 bg-zinc-950 border border-zinc-800 hover:border-amber-500 hover:text-white rounded-xl text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-2 transition-all"
            >
        ...
        {searchQuery && (
          <div className="mb-8 p-6 bg-zinc-900 border border-amber-500/40 rounded-2xl animate-fadeIn">
            <h3 className="text-sm font-bold uppercase tracking-wider text-amber-500 mb-4 flex items-center gap-2">
              <Search className="w-4 h-4" /> Wyniki wyszukiwania dla "{searchQuery}":
            </h3>
            {filteredOptions.length === 0 ? (
              <p className="text-zinc-500 text-sm font-mono">Brak dopasowań do wyszukiwanej frazy.</p>
            ) : (
                ...
                {filteredOptions.map((item) => {
                  ...
                  return (
                    <div 
                      ...
                      className="p-4 bg-zinc-950/60 border border-zinc-800 hover:border-amber-500/50 rounded-xl flex justify-between items-center cursor-pointer transition-all"
                    >
                      <div>
                      ...
                      <span className="bg-zinc-900 px-3 py-1 text-xs text-amber-500 font-mono rounded font-bold border border-zinc-800">
                        {currentValue === true ? 'WŁ.' : currentValue === false ? 'WYŁ.' : currentValue}
                      </span>
                    </div>
                  );
                })}
        ...
        {/* PRESET & SHARING SYSTEM GRID */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
          
          {/* PRESET SELECTOR (8 cols) */}
                ...
                return (
                  <button
                    ...
                    className={`text-left p-4 rounded-xl border transition-all duration-300 flex flex-col justify-between ${
                      isSelected 
                        ? 'bg-zinc-900/80 border-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.1)] text-white' 
          ...
          <section className="lg:col-span-4 p-6 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl flex flex-col justify-between space-y-4">
            <div>
              <h2 className="text-xs font-bold uppercase text-zinc-400 tracking-wider mb-2 flex items-center gap-1.5">
                <Share2 className="w-3.5 h-3.5 text-amber-500" /> Klonowanie Konfiguracji
              </h2>
              <p className="text-[11px] text-zinc-500 leading-normal font-medium text-justify">
                Wygeneruj unikalny link profilu bezpośrednio skorelowany z platformą <span className="text-amber-500 font-bold">Anonymousik.is-a.dev/scripts/warz</span>.
              </p>
            </div>

            <button
              onClick={generateShareLink}
              className="w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-zinc-950 font-black text-xs uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 transition-all shadow-md"
            >
            ...
            <div className="flex items-center gap-2 bg-zinc-950/80 border border-zinc-800 p-1 rounded-xl">
              <input 
                type="text" 
                placeholder="Wklej klucz lub cały link..."
                value={customCodeInput}
                onChange={(e) => setCustomCodeInput(e.target.value)}
                className="bg-transparent text-[11px] text-zinc-300 placeholder-zinc-600 px-3 py-1.5 w-full focus:outline-none font-mono"
              />
              <button 
                onClick={handleImportConfig}
                className="bg-zinc-800 text-white font-extrabold text-xs uppercase px-4 py-1.5 rounded-lg hover:bg-zinc-700 transition-all"
              >
                Import
              </button>
            </div>
          ...
          <aside className="lg:col-span-3 space-y-4">
                ...
                {categories.map((key) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 px-3.5 py-3 rounded-xl text-left transition-all duration-200 whitespace-nowrap lg:whitespace-normal border ${
                        isActive 
                        ? 'bg-zinc-900 border-amber-500/30 text-amber-500 shadow-[inset_4px_0_0_0_rgba(245,158,11,1)]' 
            ...
            <div className="p-5 bg-zinc-900/50 border border-zinc-800 rounded-2xl space-y-4">
              <h3 className="text-xs uppercase text-zinc-500 font-bold flex items-center gap-2">
                <Activity className="w-4 h-4 text-amber-500 animate-pulse" /> Telemetria Wydajności
              </h3>
              <div className="space-y-3 text-sm font-mono">
                <div className="flex justify-between items-center border-b border-zinc-800/40 pb-2">
                  <span className="text-zinc-500 text-[10px] uppercase">Płynność (FPS):</span>
                  <span className="text-emerald-400 font-bold text-xs">{liveMetrics.fps}</span>
                </div>
                <div className="flex justify-between items-center border-b border-zinc-800/40 pb-2">
                  <span className="text-zinc-500 text-[10px] uppercase">Czas Reakcji:</span>
                  <span className="text-amber-400 font-bold text-xs">{liveMetrics.lag}</span>
                </div>
                <div className="flex justify-between items-center border-b border-zinc-800/40 pb-2">
                  <span className="text-zinc-500 text-[10px] uppercase">Zużycie Mocy:</span>
                  <span className="text-zinc-300 font-bold text-xs">{liveMetrics.cpu}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-zinc-500 text-[10px] uppercase">Zalecany FOV:</span>
                  <span className="text-cyan-400 font-bold text-xs">{liveMetrics.fov}</span>
                </div>
              </div>
            </div>

            {/* Visual Dynamic Controllers Preview */}
            {activeTab === 'controller' && (
              <TacticalControllerVisualizer layout={settings.layout} />
            )}

            {/* Visual FOV Simulator display */}
            {activeTab === 'graphics' && (
              <FovSimulator fovValue={settings.fov} adsFov={settings.ads_fov} />
            )}
          </aside>
               ...
               <div className="flex items-center gap-1.5 bg-zinc-950 p-1.5 rounded-xl border border-zinc-800">
                 ...
                 {['PS4', 'PS5', 'PC'].map((p) => (
                   <button
                     ...
                     onClick={() => {
                       setActivePlatform(p);
                       showNotification(`Zmieniono widok ścieżek dostępu dla platformy: ${p}`, 'success');
                     }}
                     className={`px-3 py-1 text-xs font-mono font-bold rounded-lg transition-all duration-150 ${
                       activePlatform === p 
                ...
                return (
                  <div 
                    ...
                    className={`relative p-5 rounded-2xl border transition-all duration-300 flex flex-col justify-between ${
                      item.highlight 
                        ? 'bg-zinc-900/50 border-amber-500/40 hover:border-amber-500 shadow-lg' 
                  ...
                  >
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        <h3 className="font-extrabold text-zinc-100 uppercase text-xs tracking-wider pr-4">
                          {item.name}
                        </h3>
                        
                        {/* Interactive Dynamic UI elements */}
                        {item.isToggle ? (
                          <div onClick={() => toggleSetting(item.id)}>
                          ...
                          <div className="relative inline-block text-left">
                            <select 
                              ...
                              className="bg-zinc-950 border border-zinc-800 text-amber-500 font-mono text-xs px-2.5 py-1 rounded-lg focus:outline-none appearance-none cursor-pointer hover:border-zinc-700"
                            >
                              {item.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                            </select>
                          </div>
                        ) : (
                          <span className="inline-block bg-zinc-950 border border-zinc-800 text-amber-500 font-mono text-xs px-3 py-1 rounded-lg shadow-inner font-extrabold">
                            {currentValue}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-zinc-800/60 flex items-start gap-2.5">
                        ...
                        <p className={`text-xs leading-relaxed ${isAdvanced ? 'text-zinc-400 font-mono text-[11px]' : 'text-zinc-300 font-medium'}`}>
                    ...
                    <div className="mt-5">
                      ...
                      {isExpanded && (
                        <div className="mt-2.5 p-3.5 bg-zinc-950 border border-zinc-800 rounded-xl text-xs space-y-2 animate-fadeIn shadow-inner">
                          <div className="flex items-center gap-1.5 border-b border-zinc-800/60 pb-2 text-zinc-500 font-mono">
                            {activePlatform === 'PC' ? <Laptop className="w-4 h-4" /> : <Tv className="w-4 h-4" />}
                            <span>Przejdź kolejno:</span>
                          </div>
                          <p className="text-amber-400 font-extrabold leading-relaxed font-mono">
